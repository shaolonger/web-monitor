...
import 'package:web_monitor_app/modules/module_log/widgets/widget_log_chart_resource_type.dart';
...
class ScreenLogRes extends StatefulWidget {
  ...
  @override
  _ScreenLogResState createState() => _ScreenLogResState();
}

class _ScreenLogResState extends State<ScreenLogRes> {
  final String _logType = ConstLog.LOG_TYPE_LIST_MAP["RES"];
  ...
  List<dynamic> _chartResourceTypeData = [];
  ...
  void _getOverviewData() async {
    ...
    List<dynamic> result = model.resourceLoadErrorLog;
  ...
  void _getChartOverviewData() async {
    ...
    List<dynamic> chartData = model.resourceLoadErrorLog;
    setState(() {
  ...
  /// 获取环形图，资源类型数据
  void _geChartResourceTypeData() async {
    List<dynamic> resultList =
        await ServiceLog.getLogDistributionBetweenDiffDate(
      projectIdentifier: widget.projectIdentifier,
      logType: _logType,
      indicator: ConstLog.INDICATOR_LIST_MAP["RESOURCE_TYPE"],
      startTime: widget.startTime,
      endTime: widget.endTime,
    );
    setState(() {
      _chartResourceTypeData = resultList;
    });
  }
  ...
  @override
  void didUpdateWidget(covariant ScreenLogRes oldWidget) {
      ...
      children: [
        Column(
          children: [
            ...
            // 环形图，资源类型
            WidgetLogChartResourceType(_chartResourceTypeData),
          ],
        ),
      ],
    );
