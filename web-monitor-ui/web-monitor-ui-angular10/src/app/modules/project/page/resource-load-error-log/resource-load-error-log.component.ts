...
@Component({
    ...
    templateUrl: './resource-load-error-log.component.html',
    styleUrls: ['./resource-load-error-log.component.scss']
})
export class ResourceLoadErrorLogComponent implements OnInit {
    ...
    logTimeChartFilterForm = {
        ...
        startTime: '',
        endTime: '',
        logTypeList: 'resourceLoadErrorLog',
        indicatorList: 'count',
        timeInterval: 60
    };
    chartTimeRangeModel = [];
    ...
    // 图表-资源类型
    chartOptionResourceType: EChartOption = {
        tooltip: {
            trigger: 'item',
            formatter: '{a} <br/>{b}: {c} ({d}%)'
        },
    ...
    getTableList(): void {
        ...
        this.isLoading = true;
        this.logService.getResourceLoadErrorLogByGroup(
            this.tableFilterForm,
            res => {
            ...
            err => {
                console.log('[失败]获取resourceLoad异常日志', err);
                this.isLoading = false;
            }
        );
    }
