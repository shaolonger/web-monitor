...
@Component({
    selector: 'app-custom-error-log',
    templateUrl: './custom-error-log.component.html',
    styleUrls: ['./custom-error-log.component.scss']
})
export class CustomErrorLogComponent implements OnInit {
    ...
    // 图表-自定义异常
    logTimeChartOptionCustom: EChartOption = {
        tooltip: {
            trigger: 'axis',
            axisPointer: {
                type: 'cross',
    ...
    getTotalData(): void {
        ...
        let formData = {
            projectIdentifier: this.projectIdentifier,
            startTime, endTime, logTypeList: 'customErrorLog', indicatorList: 'count,uv', timeInterval: 86400
        };
    ...
    setChartTotal(data: any): void {
        let { customErrorLog } = data;
        let yesterday = customErrorLog[0];
        let today = customErrorLog[1];
    ...
    setLogTimeChartsOption(data: any): void {
        let { customErrorLog } = data;
        // 设置自定义异常图表
        this.logTimeChartOptionCustom.xAxis[0].data = customErrorLog.map(item => item.key);
        this.logTimeChartOptionCustom.series[0].data = customErrorLog.map(item => item.count);
        this.logTimeChartOptionCustom = { ...this.logTimeChartOptionCustom };
    }
    ...
    getTableList(): void {
        ...
        this.isLoading = true;
        this.logService.getCustomErrorLogByGroup(
            this.tableFilterForm,
            res => {
                console.log('[成功]获取custom异常日志', res);
                this.isLoading = false;
                let { success, data, msg } = res;
                if (!success) {
                    this.message.error(msg || '获取custom异常日志失败');
                } else {
            ...
            err => {
                console.log('[失败]获取custom异常日志', err);
                this.isLoading = false;
            }
        );
    }
