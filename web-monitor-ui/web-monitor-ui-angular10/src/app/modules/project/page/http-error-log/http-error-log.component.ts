...
@Component({
    selector: 'app-http-error-log',
    templateUrl: './http-error-log.component.html',
    styleUrls: ['./http-error-log.component.scss']
})
export class HttpErrorLogComponent implements OnInit {
    ...
    // 图表-Http异常
    logTimeChartOptionHttp: EChartOption = {
        tooltip: {
            trigger: 'axis',
            axisPointer: {
                type: 'cross',
    ...
    // 图表-状态码
    chartOptionStatus: EChartOption = {
        tooltip: {
            trigger: 'item',
            formatter: '{a} <br/>{b}: {c} ({d}%)'
        },
    ...
    getTotalData(): void {
        ...
        let formData = {
            projectIdentifier: this.projectIdentifier,
            startTime, endTime, logTypeList: 'httpErrorLog', indicatorList: 'count,uv', timeInterval: 86400
        };
    ...
    setChartTotal(data: any): void {
        let { httpErrorLog } = data;
        let yesterday = httpErrorLog[0];
        let today = httpErrorLog[1];
    ...
    setLogTimeChartsOption(data: any): void {
        let { httpErrorLog } = data;
        // 设置Http异常图表
        this.logTimeChartOptionHttp.xAxis[0].data = httpErrorLog.map(item => item.key);
        this.logTimeChartOptionHttp.series[0].data = httpErrorLog.map(item => item.count);
        this.logTimeChartOptionHttp = { ...this.logTimeChartOptionHttp };
    }
    ...
    getDistributionChartData(): void {
        const indicatorList = ['device_name', 'os', 'browser_name', 'net_type', 'status'];
    ...
    setDistributionChartsOption(indicator: string, data: any): void {
        switch (indicator) {
            ...
            case 'net_type':
                ...
                break;
            case 'status':
                this.chartOptionStatus.series[0].data = data.map(item => ({
                    name: item.key,
                    value: item.count
                }));
                this.chartOptionStatus.graphic.style.text = data.reduce((max, cur) => max + cur.count, 0);
                this.chartOptionStatus = { ...this.chartOptionStatus };
                break;
            default:
                break;
        }
    ...
    getTableList(): void {
        ...
        this.isLoading = true;
        this.logService.getHttpErrorLogByGroup(
            this.tableFilterForm,
            res => {
                console.log('[成功]获取http异常日志', res);
                this.isLoading = false;
                let { success, data, msg } = res;
                if (!success) {
                    this.message.error(msg || '获取http异常日志失败');
                } else {
